...
export class PersistentIndexManager {
  ...
  async open(): Promise<void> {
    await this.enqueue(async () => {
      ...
      if (
        snapshot.manifest.pluginVersion !== this.pluginVersion
        || snapshot.manifest.vaultFingerprint !== this.vaultFingerprint
        || !modelsEqual(snapshot.manifest.model, this.embeddingClient?.descriptor ?? null)
      ) {
        this.clearMemory();
      } else {
  ...
  async reconcile(): Promise<void> {
    await this.enqueue(async () => {
      ...
      const pending = files.filter((file) => {
        ...
        return this.documents.get(file.path)?.modifiedAt !== file.stat.mtime;
      });
  ...
  scheduleScopeRebuild(delayMs = 500): void {
    ...
    void this.enqueue(async () => {
      if (!this.getSettings().semanticIndexingEnabled) {
        this.updateReadyStatus();
        return;
      }
  ...
  private async processFiles(
    ...
    for (const file of files) {
      ...
      if (result === null) {
        ...
        if (previous?.contentHash === result.document.contentHash) {
  ...
  private replaceDocument(parsed: ParsedIndexDocument): void {
    this.removeFromMemory(parsed.document.path);
  ...
  private createSnapshot(completedAt: number): IndexSnapshot {
    ...
    return {
      manifest: {
        ...createEmptyIndexManifest(this.pluginVersion, this.vaultFingerprint),
        model,
        dimensions,
  ...
  private canIndex(): boolean {
    return this.opened
      && !this.disposed
      && this.getSettings().semanticIndexingEnabled;
  }
  ...
  private updateReadyStatus(message?: string): void {
    ...
    this.updateStatus("ready", message ?? vectorMessage, {
      lastCompletedAt: this.generation === 0 ? null : Date.now()
    });
  }
...
function createScopeSignature(settings: SemanticLinksSettings): string {
  return JSON.stringify([
    settings.semanticIndexingEnabled,
...
function modelsEqual(
  ...
  right: EmbeddingClient["descriptor"] | null
): boolean {
