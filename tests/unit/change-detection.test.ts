...
test("reuses the whole document when a reread has the same hash", () => {
  assert.equal(canReuseDocument(document, "same-content"), true);
  assert.equal(canReuseDocument(document, "different-content"), false);
});

test("removes only chunks that disappeared from a modified note", () => {
  assert.deepEqual(
    findRemovedChunkIds(document, ["chunk-a", "chunk-c", "chunk-d"]),
    ["chunk-b"]
  );
  assert.deepEqual(findRemovedChunkIds(undefined, ["chunk-a"]), []);
});
